...
const runMigrationsAndSeeders = async () => {
  try {
    ...
    console.log(
      ">Basic-setup| [run-migration-seeders.js] | LINE #34 | error : ",
      error
    );
    throw error;
  }
...
const getMigratInstance = async function (folderName = "migrations") {
  let time = Date.now();
  const sequelize = await getSequelizeIns();
  const migrator = new Umzug({
    ...
    storage: new SequelizeStorage({
      sequelize,
      modelName:
        folderName === "migrations" ? "system_migrations" : "system_seeders",
    }),
  });
  migrator.on("migrating", ({ name }) => {
    ...
    console.log(
      `== ${name}: ${
        folderName === "migrations" ? "migrating" : "seeding"
      } =======`
    );
  });
  migrator.on("migrated", ({ name }) => {
    console.log(
      `== ${name}: ${folderName === "migrations" ? "migrated" : "seeded"} (${
        (Date.now() - time) / 1000
      }s) \n`
    );
  });
  return {
    [folderName === "migrations" ? "migratorConfig" : "seederConfig"]: migrator,
    sequelize,
  };
};
