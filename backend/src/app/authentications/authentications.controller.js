...
const {
  ...
  throwError,
  throwIfNot,
} = require("../../services/throw-error-class");
...
const signUp = async (req) => {
  ...
  if (
    ...
    !mobileRegex.test(req.body.mobileNumber)
  ) {
    throwError(statusCodes.BAD_REQUEST, statusMessages.INVALID_DETAILS);
  }
  const userAlreadyExists = await userService.userAlreadyExists({
    [Op.or]: [
  ...
  throwIf(
    ...
    statusMessages.USER_ALREADY_EXIST
  );
...
const login = async (req, res) => {
  ...
  const inputPassword = authService.encryptPassword(
    password,
    isExists.userSalt
  );
  ...
  if (inputPassword === isExists.password) {
    const token = authService.issueJwt({
      ...JSON.parse(JSON.stringify(isExists)),
    });
