...
import { activeID } from '../../../../../store/reducers/menuReducer';
...
const NavGroup = ({
  ...
  const navCollapse = item.children?.map((menuItem) => {
    switch (menuItem.type) {
      ...
      case 'item':
        return <NavItem handleDrawerToggle={handleDrawerToggle} key={menuItem.id} item={menuItem} level={1} />;
      default:
        return null;
    }
  });
  ...
  const items = currentItem.children?.map((menuItem) => {
    switch (menuItem.type) {
      case 'collapse':
        return (
          <NavCollapse
          ...
          >
            {itemIcon && (
              <ListItemIcon sx={{ minWidth: 28 }}>
                {currentItem.id === lastItemId ? <IconChevronDown style={{ fontSize: 20, stroke: '1.5' }} /> : itemIcon}
              </ListItemIcon>
            )}
            <ListItemText
              sx={{ mr: 1 }}
            ...
            />
            {openMini ? (
              <IconChevronDown style={{ fontSize: 16, stroke: '1.5' }} />
            ) : (
              <IconChevronRight style={{ fontSize: 16, stroke: '1.5' }} />
            )}
            {anchorEl && (
              <PopperStyled
                id={popperId}
