...
export const generateGaaApiParameter = ({ areaLevelsData, values, methods, accessRank }) => {
  let newLevelFilter = {};
  let hierarchyLevelId = {};
  let errorFlag = false;
  if (areaLevelsData) {
    for (let i = areaLevelsData?.length - 1; i >= 0; i--) {
      if (values[`gaaLevelEntryId${i}`]?.length) {
        const columnName = areaLevelsData[i]?.columnName;
        newLevelFilter[columnName] = values[`gaaLevelEntryId${i}`];
        hierarchyLevelId = areaLevelsData[i]?.id;
        break;
      }
    }
  ...
  return { errorFlag, newLevelFilter, hierarchyLevelId };
};
